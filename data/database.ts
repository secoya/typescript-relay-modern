...
// Mock todo data
const todosById: { [key: string]: Todo } = {};
const todoIdsByUser: { [key: string]: string[] } = {
	[VIEWER_ID]: [],
};
let nextTodoId = 0;
...
export function changeTodoStatus(id: string, complete: boolean): void {
...
export function getTodos(status: 'any' | 'completed' | 'not completed' = 'any'): Todo[] {
...
export function getViewer(): User {
	return getUser(VIEWER_ID) as User;
}

export function markAllTodos(complete: boolean): string[] {
...
export function removeCompletedTodos(): string[] {
...
export function renameTodo(id: string, text: string): void {
	const todo = getTodo(id);
	todo.text = text;
}
